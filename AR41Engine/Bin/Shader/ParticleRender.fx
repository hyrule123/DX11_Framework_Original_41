...
void ParticleGS(point VS_OUTPUT_PARTICLE input[1],
    inout TriangleStream<GS_OUTPUT_PARTICLE> output)
{
    ...
    float3  Scale = lerp(g_ParticleShare[0].ScaleMin,
        g_ParticleShare[0].ScaleMax, float3(Ratio, Ratio, Ratio));

    float4  Color = lerp(g_ParticleShare[0].ColorMin,
        g_ParticleShare[0].ColorMax, float4(Ratio, Ratio, Ratio, Ratio));
