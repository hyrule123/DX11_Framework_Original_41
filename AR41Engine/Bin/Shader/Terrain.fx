...
float3 ComputeBumpNormalTerrain(float3 Tangent, float3 Binormal,
    float3 Normal, float2 UV, float2 OriginUV)
{
    ...
    if (g_MtrlAmbientColor.w == 1.f)
    {
        ...
        for (int i = 0; i < g_TerrainSplatCount; ++i)
        {
            ...
            NormalColor.rgb = NormalColor.rgb * (1.f - Alpha.r) +
                SplatNrm.rgb * Alpha.r;
        }
...
PS_OUTPUT_GBUFFER TerrainPS(VS_OUTPUT_STATIC input)
{
    ...
    for (int i = 0; i < g_TerrainSplatCount; ++i)
    {
        ...
        SpecularColor.rgb = SpecularColor.rgb * (1.f - Alpha.r) +
            SplatSpc.rgb * Alpha.r;
    }
