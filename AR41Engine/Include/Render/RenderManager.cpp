...
void CRenderManager::Render3D(float DeltaTime)
{
	...
	for(; iter != iterEnd; ++iter)
	{
		...
		{
			bool Frustum = Camera->FrustumInSphere((*iter1)->GetWorldPos(),
				(*iter1)->GetRadius());

			(*iter1)->SetFrustumCull(!Frustum);
		}
	}*/
...
void CRenderManager::RenderShadowMap(float DeltaTime)
{
	...
	UINT	Count = 1;
	CDevice::GetInst()->GetContext()->RSGetViewports(&Count,
		&PrevVp);
	...
	CDevice::GetInst()->GetContext()->RSSetViewports(1, 
		&PrevVp);
}
...
void CRenderManager::RenderParticle(float DeltaTime)
{
	...
	for (; iter != iterEnd;)
	{
		...
		if ((*iter)->GetSceneComponentType() ==
			SceneComponentType::Primitive &&
			!(*iter)->CheckTypeID<CParticleComponent>())
		{
			CMesh* Mesh = ((CPrimitiveComponent*)iter->Get())->GetMesh();
...
void CRenderManager::CreateRenderTarget()
{
	...
	CResourceManager::GetInst()->CreateTarget("GBuffer5", RS.Width,
		RS.Height, DXGI_FORMAT_R32G32B32A32_FLOAT);

	CResourceManager::GetInst()->CreateTarget("GBuffer6", RS.Width,
		RS.Height, DXGI_FORMAT_R32G32B32A32_FLOAT);
	...
	CResourceManager::GetInst()->CreateTarget("ShadowMap", m_ShadowMapRS.Width,
		m_ShadowMapRS.Height, DXGI_FORMAT_R32G32B32A32_FLOAT,
	...
	m_ShadowCBuffer->SetShadowResolution(Vector2((float)m_ShadowMapRS.Width,
		(float)m_ShadowMapRS.Height));
