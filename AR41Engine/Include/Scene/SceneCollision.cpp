...
bool CSceneCollision::Picking(PickingResult& result)
{
	...
	for (size_t i = 0; i < SectionCount; ++i)
	{
		...
		if (CCollisionManager::GetInst()->CollisionRayToSphere(
			SectionResult, ray, Center, Radius))
		{
	...
	for (; iter != iterEnd; ++iter)
	{
		...
		{
			if (!(*iter1)->GetActive() || !(*iter1)->GetEnable() ||
				(*iter1)->GetFrustumCull())
				continue;

			if (CCollisionManager::GetInst()->CollisionRayToSphere(
				result, ray, (*iter1)->GetCenter(), 
				(*iter1)->GetRadius()))
			{
...
bool CSceneCollision::SortColliderList(
	CSharedPtr<CCollider3D> Src, 
	CSharedPtr<CCollider3D> Dest)
{
