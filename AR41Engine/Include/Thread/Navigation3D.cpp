...
bool CNavigation3D::FindNode(NavigationCell* Node, NavigationCell* StartNode,
	NavigationCell* EndNode, const Vector3& End,
	std::list<Vector3>& PathList)
{
	...
	for (size_t i = 0; i < AdjSize; ++i)
	{
		...
		if (m_vecCell[AdjIndex]->NodeType == ENavNodeType::None)
		{
			...
			m_vecCell[AdjIndex]->Total = m_vecCell[AdjIndex]->Cost +
				m_vecCell[AdjIndex]->Dist;
