...
bool CParticle::Init()
{
	...
	AddStructuredBuffer("ParticleInfo", sizeof(ParticleInfo),
		m_CBuffer->m_BufferData.ParticleSpawnCountMax,
		0);

	AddStructuredBuffer("ParticleInfoShare", sizeof(ParticleInfoShare),
		1, 1);
...
void CParticle::AddStructuredBuffer(const std::string& Name,
	...
	bool Dynamic, int StructuredBufferShaderType)
{
	...
	if (!Buffer->Init(Name, Size, Count, Register, Dynamic,
		StructuredBufferShaderType))
	{
		SAFE_DELETE(Buffer);
		return;
	}
...
void CParticle::ResizeStructuredBuffer(const std::string& Name,
	unsigned int Size, unsigned int Count, int Register,
	bool Dynamic, int StructuredBufferShaderType)
{
	...
	for (size_t i = 0; i < BufferCount; ++i)
	{
		...
		{
			m_vecStructuredBuffer[i]->Init(Name, Size, Count, Register, Dynamic,
				StructuredBufferShaderType);
		}
	}
}
...
void CParticle::SetParticleSpawnCountMax(int Count)
{
	...
	ResizeStructuredBuffer("ParticleInfo", sizeof(ParticleInfo),
		Count,
		0);
}
