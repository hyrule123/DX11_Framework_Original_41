...
bool CTerrainComponent::SetMeshFullPath(const std::string& Name,
...
void CTerrainComponent::CreateTerrain( 
	int CountX, int CountY,
	float SizeX, float SizeY, const TCHAR* HeightMapName, 
	const std::string& HeightMapPath)
{
	...
	if (HeightMapName)
	{
		...
		WideCharToMultiByte(CP_ACP, 0, FullPath, -1, FullPathMultibyte,
		...
		_splitpath_s(FullPathMultibyte, nullptr, 0, nullptr, 0, nullptr, 0,
			Ext, _MAX_EXT);
		_strupr_s(Ext);
	...
	for (int i = 0; i < m_CountY; ++i)
	{
		...
		{
			Vertex3DStatic	Vtx;
			Vtx.Pos = Vector3((float)j * m_CellSize.x, 
				vecY[i * m_HeightMapX + j],
				(m_Size.y - m_CellSize.y) - (float)i * m_CellSize.y);
			//Vtx.Normal = Vector3(0.f, 1.f, 0.f);
			Vtx.UV = Vector2((float)j / (float)(m_CountX - 1),
				(float)i / (float)(m_CountY - 1));
